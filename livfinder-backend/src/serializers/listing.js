...
export function serializeListingCard(row) {
  ...
  return {
    ...
    // The full timestamp, not just the day: the website reads this as "2 hours ago",
    // which a day-only value cannot express for anything posted today.
...
export function serializeListingDetail({ listing, detail, media = [], features = [], attributes = [], organization, agent, project }) {
  ...
  return {
    ...
    publishedAt: isoDate(listing.published_at),
